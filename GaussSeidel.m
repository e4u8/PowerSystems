function [Bus_matrix_v_plus_1, Generators_matrix, P_total_Loads, P_total_Losses] = GaussSeidel(Bus_matrix_v,Bus_matrix_v_plus_1,Lines_matrix,Generators_matrix, Cost_matrix, Y_final, Z, B)

% Maybe we don't need Cost_matrix
% remove disp()
...
%                               disp(v)
     ...
     if(Delta_V_max <= e_threshold)          % if algorithms converges
          ...
          for k = 1:1:length(Lines_matrix)            
               ...
               S_ij_pu(k) = Vi_phasor_pu * (conj(Vi_phasor_pu) - conj(Vj_phasor_pu)) * ( 1 / conj(Z(k)) ) + ( (Vi_real_pu^2) *  (conj(B(k))/2 ));          % formula Sij for pu
               S_ji_pu(k) = Vj_phasor_pu * (conj(Vj_phasor_pu) - conj(Vi_phasor_pu)) * ( 1 / conj(Z(k)) ) + ( (Vj_real_pu^2) *  (conj(B(k))/2 ));          % formula Sji for pu
          ...
          P_total_Loads = sum(Bus_matrix_v_plus_1(:,3));  
          P_total_Losses = P_losses_ij;
